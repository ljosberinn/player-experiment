...
describe("what a row has to say about itself", () => {
  it("says nothing about an ordinary track", () => {
    ...
    expect(rowStatus(track(), 2)).toBeNull();
  });

  it("marks the track that is playing", () => {
    expect(rowStatus(track({ id: 7 }), 7)).toBe("playing");
  });

  it("marks a track whose file is gone", () => {
    expect(rowStatus(track({ missing_since: 1_700_000_000 }), null)).toBe("missing");
  });
  ...
  it("prefers playing over a stale missing mark", () => {
    ...
    expect(rowStatus(track({ id: 7, missing_since: 1_700_000_000 }), 7)).toBe("playing");
  });
  ...
  it("treats a zero timestamp as missing rather than as absent", () => {
    ...
    expect(rowStatus(track({ missing_since: 0 }), null)).toBe("missing");
  });
});
