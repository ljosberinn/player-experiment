...
**23 — In-app updates** `feat/23-updater`
Now that CI publishes installers, the app should be able to notice and install
them. `tauri-plugin-updater` does the mechanics; the decisions are what make
this a phase.

**⚠ Two things must be settled before any of it is written.**

1. **The repository is private.** The updater fetches a manifest and an
   installer from a URL. GitHub release assets on a private repo require an
   authenticated request, and the only ways to give a shipped binary that
   credential are to embed a token in it - which puts a repo credential on
   every machine the app is installed on, and it is readable - or to proxy
   through a server, which this project does not have and should not need.
   The realistic options are: **make the repo public**, **host the manifest
   and installers somewhere public** while the source stays private, or **do
   not ship auto-update**. This is the user's call and nothing else can start
   until it is made.
2. **Updates must be signed, and that is separate from code signing.** Tauri's
   updater refuses an unsigned artifact. The key is a **minisign** keypair
   generated by `tauri signer generate` - free, no certificate authority, and
   unrelated to the Authenticode signing that was ruled out at the start. The
   public key goes in `tauri.conf.json`; the private key and its password
   become repository secrets that the release workflow needs, and losing the
   private key means no existing install can ever update again. Worth
   generating deliberately and backing up somewhere that is not this repo.

**Which of the three behaviours.** The user offered notify-then-install,
background-download-then-prompt, and background-download-then-apply-on-close.
The recommendation is a blend of the last two, and specifically **not** silent
application:

- Check on launch and then every few hours, on a timer, quietly. A failed
  check is a no-op, not an error - being offline is normal.
- Download in the background once something is found. This is the part worth
  hiding: it is slow, it needs no decision, and asking first only makes the
  user wait afterwards.
- Then say so, quietly, in the footer beside the version: *"0.3.0 ready -
  restart to install"*, clickable to do it now.
- **Apply on quit, not on close-to-background, and never mid-session.** A
  music player that restarts itself while a song is playing is a bug however
  it is described. Applying on quit means the next launch is already the new
  version, which is the behaviour that feels like it did nothing at all -
  which is the goal.
- A "Check for updates" item belongs wherever settings eventually live, for
  the case where someone wants it now.

**What the release workflow needs.** `tauri build` produces the updater
artifacts and their `.sig` files only when the updater plugin is configured
and the signing key is present in the environment; the release job then
uploads `latest.json` alongside the installers. That file is what the
`endpoints` in `tauri.conf.json` point at.

*Testing:* the version-comparison and "is there anything newer" logic is pure
and testable; the plugin calls mock cleanly for the state machine (idle →
checking → downloading → ready → failed). What cannot be tested in CI is the
