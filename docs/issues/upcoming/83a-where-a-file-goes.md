...
*Open:* where a truncated segment is cut. Recommendation: a hard cut on a
character boundary with no ellipsis — a truncated folder name is already
visibly truncated, and a marker only spends budget. Also: a root deep enough to
leave no budget at all is refused when it is picked, in
[83c](83c-turning-the-library-folder-on.md), rather than truncated to nothing
here.
