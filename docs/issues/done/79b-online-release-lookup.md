...
- **The rate limiter is process-wide, not per client.** Max 1 request/sec is
  ...
  client instance lets [82](../upcoming/82-lookup-runs-itself.md)'s background pass and an
...
[82](../upcoming/82-lookup-runs-itself.md) needs the number and cannot invent it later.
