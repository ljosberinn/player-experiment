...
The filters persist in `settings` under one `stats.filters` key holding opaque
JSON, the way `sidebar.sections` does: which filters exist is the frontend's
business, and a constant with a command pair per filter would be three things to
add every time a panel grows a facet. The drill path
...
its filter bar draws. **The tile rows moved from 84 into 80**, because a shell
with no panel in it has no caller for the filter bar it ships, and the two
`*_totals` aggregates are the only panels needing no primitive 70 declined to
build. **84 split into a phase per tab when the
...
**The Listening half then split again, along the primitive seam rather than the
tab.** 70's rule is that each primitive lands with a caller, and the two kinds of
panel want different primitives: a ranked list is HTML and a share, a time series
is SVG over `scales.ts`. So 84a is `BarList` and its callers and 84c is `Bar`,
`Heatmap` and theirs, and neither phase ships a primitive it does not draw with.
...
- **Which panels earn their place** is answered by using them, not by this
  document. The calendar year heatmap is the first to go, in 84c, for wanting a
  year selector nothing else in the view has.
- **CSV export mechanics** were decided in 80 and are a frontend path — rows the
  panel already holds to a string, and the shell's save dialog — **plus a
  `save_text_file` command**, because `@tauri-apps/plugin-fs` is not a
  dependency and nothing in the frontend can otherwise write the bytes.
  `export/`'s generator walks the whole library behind `export://progress`,
  which is the opposite problem. It lands with its first caller in 84a.
