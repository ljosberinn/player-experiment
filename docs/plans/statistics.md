...
- **CSV export mechanics** were decided in 80 and are **a small frontend path**
  — rows the panel already holds to a string, and the shell's save dialog.
  `export/`'s generator walks the whole library behind `export://progress`,
  which is the opposite problem. It lands with its first caller in 84b.
