...
/// Exports what `compile_rule` accepts, for the editor to check itself against.
///
/// The editor offers a deliberate *subset*: `OPS_BY_KIND` in `filterTree.ts`
/// leaves out "is" and "is not" on a timestamp, because matching a date against
/// an exact unix second is not something to offer anybody. So the two tables
/// cannot be generated from one another - but the subset relation can be
/// asserted, and that needs the accepted set as data.
///
/// The assertion lives in `filterTree.test.ts` rather than here, because it
/// belongs on the side that might be wrong. This module only writes the table.
///
/// Named `export_bindings_*` so `npm run bindings` regenerates it along with
/// ts-rs's output and CI's staleness check on `src/ipc/bindings` covers it.
#[cfg(test)]
mod bindings {
    ...
    use super::compile;
    use crate::model::{
        Combinator, FilterField, FilterFieldKind, FilterGroup, FilterNode, FilterOp, FilterRule,
        FilterValue,
    };

    /// Every variant of a camelCase unit enum, in declaration order.
    ///
    /// Read back out of the ts-rs declaration rather than hand-listed, because
    /// the derive sees the enum arms and a `const ALL` would only see whoever
    /// last remembered it. A ts-rs format change makes the parse fail rather
    /// than quietly exporting a short table.
    fn variants<T: TS + DeserializeOwned>() -> Vec<T> {
        let config = Config::from_env();
        T::inline(&config)
            .split('|')
            .map(|token| {
                serde_json::from_str(token.trim()).unwrap_or_else(|error| {
                    panic!("{} is not a camelCase unit enum: {error}", T::name(&config))
                })
            })
            .collect()
    }

    /// The camelCase name a variant crosses the IPC boundary as.
    fn name<T: Serialize>(value: &T) -> String {
        match serde_json::to_value(value).unwrap() {
            serde_json::Value::String(name) => name,
            other => panic!("expected a unit enum to serialize as a string, got {other}"),
        }
    }

    /// Whether the backend accepts this combination at all.
    ///
    /// Every shape of value is tried, and one compiling is enough: a rejected
    /// operator and a value of the wrong shape are both `AppError::Internal`,
    /// so asking the question this way avoids matching on error strings.
    fn accepts(field: FilterField, op: FilterOp) -> bool {
        let candidates = [
            FilterValue::Text {
                text: "x".to_owned(),
            },
            FilterValue::Number { number: 1 },
            FilterValue::Range { from: 1, to: 2 },
            FilterValue::None,
        ];
        candidates.into_iter().any(|value| {
            ...
            compile(&group, 0).is_ok()
        })
    }

    /// Where ts-rs writes, asked of ts-rs rather than worked out again, so the
    /// table cannot land anywhere but beside the bindings it imports.
    fn export_dir() -> PathBuf {
        Config::from_env().out_dir().to_owned()
    }

    #[test]
    fn export_bindings_filter_ops() {
        let ops = variants::<FilterOp>();
        let mut kinds: Vec<(String, String)> = Vec::new();
        let mut accepted_by_kind: BTreeMap<FilterFieldKind, Vec<FilterOp>> = BTreeMap::new();

        for field in variants::<FilterField>() {
            let kind = field.kind();
            let accepted: Vec<FilterOp> = ops
                .iter()
                .copied()
                .filter(|op| accepts(field, *op))
                .collect();

            // `compile_rule` matches on (kind, op) alone, which is why the
            // exported table is keyed by kind. Asserting it rather than
            // assuming it means a rule added for one field in particular
            // fails here instead of being averaged away.
            match accepted_by_kind.get(&kind) {
                Some(existing) => assert_eq!(
                    existing, &accepted,
                    "{field:?} accepts different operators than the other {kind:?} fields"
                ),
                None => {
                    accepted_by_kind.insert(kind, accepted);
                }
            }

            kinds.push((name(&field), name(&kind)));
        }

        let mut out = String::from(
            r#"// This file was generated by `cargo test export_bindings_filter_ops`. Do not edit this file manually.
...
"#,
        ...
        for (kind, accepted) in &accepted_by_kind {
            let names: Vec<String> = accepted
                .iter()
                .map(|op| format!("\"{}\"", name(op)))
                .collect();
            writeln!(out, "  {}: [{}],", name(kind), names.join(", ")).unwrap();
        }
        out.push_str("};\n");

        std::fs::write(export_dir().join("filterOps.generated.ts"), out).unwrap();
    }
}
