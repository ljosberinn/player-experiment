...
/// Every batch id in the journal, oldest first.
fn batch_ids(db: &Db) -> Vec<i64> {
    let conn = db.conn().unwrap();
    let mut statement = conn
        .prepare("SELECT DISTINCT batch_id FROM tag_undo ORDER BY batch_id")
        .unwrap();
    let ids = statement
...
/// Sets one track's title, as a batch of its own.
fn retitle(db: &Db, track_id: i64, title: &str, now: i64) {
    let mut conn = db.conn().unwrap();
    write::apply(
        &mut conn,
        &[(
...
/// Uncapped, the journal is one row per track per edit forever - and the
/// unattended lookup pass of phase 82b would leave tens of thousands behind,
/// with an Edit ▸ Undo enabled over all of them.
#[test]
fn the_journal_keeps_only_the_last_batches() {
    let h = harness();
    let track = id_of(&h.db, "Maki");
    let batches = write::UNDO_BATCHES + 10;

    for n in 0..batches {
        retitle(&h.db, track, &format!("Edit {n}"), 0);
    }

    assert_eq!(
        batch_ids(&h.db).len() as i64,
        write::UNDO_BATCHES,
        "the journal is capped at its newest batches, not left to grow"
    );
    ...
    assert_eq!(
        rows,
        write::UNDO_BATCHES,
        "the trimmed batches took their rows with them"
    );
}

/// The defect the cap would otherwise introduce. `batch_id` used to be derived
/// from the journal's height, which trimming lowers - so two batches in the
/// same second could be handed the same id, and `undo_last` takes the largest.
#[test]
fn batch_ids_keep_rising_even_though_trimming_shrinks_the_journal() {
    let h = harness();
    let track = id_of(&h.db, "Maki");
    let batches = write::UNDO_BATCHES + 10;

    // Every batch in the same second, which is what makes a height-derived id
    // collide rather than merely repeat a timestamp.
    for n in 0..batches {
        retitle(&h.db, track, &format!("Edit {n}"), 0);
    }

    // One row per batch here, so a journal holding more rows than it holds ids
    // is two batches that were handed the same one.
    let conn = h.db.conn().unwrap();
    let rows: i64 = conn
        .query_row("SELECT count(*) FROM tag_undo", [], |row| row.get(0))
        .unwrap();
    ...
    assert_eq!(
        batch_ids(&h.db).len() as i64,
        rows,
        "two batches were given the same id"
    );

    // What the user would see: Undo takes back the last edit made, rather than
    // an older one that shares its id.
    let mut conn = h.db.conn().unwrap();
    write::undo_last(&mut conn, |_| {}).unwrap();
    assert_eq!(
        tags::read(&path_of(&h.db, track)).unwrap().title.as_deref(),
        Some(format!("Edit {}", batches - 2).as_str()),
        "undo reverted something other than the newest batch"
    );
}

/// The floor half of the derivation: with the journal behind the clock, an id
/// still reads as when the edit happened.
#[test]
fn a_batch_id_is_a_timestamp_when_the_journal_is_older_than_the_clock() {
    let h = harness();
    let track = id_of(&h.db, "Maki");
    let when = 1_700_000_000;

    retitle(&h.db, track, "First", 0);
    retitle(&h.db, track, "Second", when);

    assert_eq!(
        batch_ids(&h.db).last().copied(),
        Some(when * 1000),
        "the newest id is the second it was written in, not one past the last id"
    );
}
